...
- ⏳ **Build רשמי ל-iOS רץ (2026-09-16)** — המשתמש הריץ בעצמו בטרמינל האמיתי שלו (לא דרך Claude, כי דרש Apple ID+2FA). נוצרו אוטומטית: Distribution Certificate, Provisioning Profile, Apple Push Notifications key. Build #5, ID `ffdac6e5-05eb-4002-8e63-985c4e891eb7`. חשוב: **build:configure/build הראשוני חייב לרוץ בטרמינל אמיתי של המשתמש** — נכשל דרך הכלים של Claude כי אין שם TTY אמיתי לפרומפטים אינטראקטיביים (`Distribution Certificate is not validated for non-interactive builds`).
