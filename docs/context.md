...
- ⏳ **build #6 רץ (2026-09-16, 18:24)** — כולל את תיקוני ATT + Sign in with Apple. ID: `f1c1b5f9-e61f-4044-b7a9-345135b7fac7`. **החלטה חשובה מהמשתמש: זה יהיה ה-build האחרון לפני submit** — EAS Free plan מוגבל במספר builds/חודש, לא בונים שוב סתם על כל שינוי קטן. ⚠️ סיכון פתוח: ה-build רץ non-interactive ודילג על אימות Provisioning Profile מול אפל ("Skipping Provisioning Profile validation... because we aren't authenticated") — ייתכן שהפרופיל לא כולל את יכולת ה-Sign in with Apple החדשה. צריך לבדוק בפועל (להתקין את ה-build ולבדוק שכפתור Apple עובד) לפני שסומכים עליו כ-build סופי.
- ⬜ **נשאר לעשות**: לבדוק את build #6 בפועל (כולל אם Sign in with Apple עובד), צילומי מסך אמיתיים, תזכורת יומן לחידוש Apple client secret (מרץ 2027).
- ⬜ לוודא שה-build הופיע ב-TestFlight בפועל ולהתחיל בדיקה פנימית.
