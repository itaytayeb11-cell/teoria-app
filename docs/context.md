...
- 👤 **ממתין לך — 2 טוגלים ידניים בהגדרות GitHub (חד-פעמי, 2 דק')**:
  1. הפוך את הריפו מ-Private ל-Public: https://github.com/itaytayeb11-cell/teoria-app/settings → גלול ל-"Danger Zone" → "Change visibility" → Public. (נבדק מראש: אין שום מפתח/סוד בקוד המחובר — רק `.env.example` עם placeholders, כל שאר קבצי env ב-`.gitignore`)
