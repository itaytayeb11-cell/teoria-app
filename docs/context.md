...
- ⬜ **נשאר לעשות**: **build חדש** ל-iOS שיכלול את תיקוני ATT + Sign in with Apple (build #5 הקיים ב-TestFlight לא כולל אותם), צילומי מסך אמיתיים (עדיין לא נפתר — האוטומציה על הסימולטור לא אמינה), Secondary category (אופציונלי), Copyright field, תזכורת יומן לחידוש Apple client secret (מרץ 2027).
