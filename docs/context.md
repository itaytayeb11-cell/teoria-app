...
- ✅ **DSA Trader status** — המשתמש בחר "I'm **not** a trader under the DSA" (אחרי דיון על הסיכונים — קטנים ורחוקים לאפליקציה בהיקף כזה). בוצע דרך Claude-in-Chrome.
- 🔴 **תזכורת קריטית לזכור**: ⚠️ אין לי יכולת תזמון אמיתית ל-6 חודשים קדימה (ScheduleWakeup/CronCreate שניהם session-only, נעלמים כשהשיחה נגמרת) — **המשתמש צריך לשלוח בעצמו** את הפרומפט ליצירת תזכורת יומן Google לחידוש Apple client secret (18.3.2027) — עדיין לא אושר שנשלח בפועל.
- ⬜ **נשאר לעשות**: לוודא build #8 מופיע ב-TestFlight ולבדוק שלא קורס (הבדיקה הקריטית ביותר כרגע) + Sign in with Apple/Google/ATT + לצלם מסכים אמיתיים → לוודא ששליחת תזכורת היומן בוצעה בפועל → למלא צילומי מסך בדף החנות → Submit ל-Review.
