...
- ⬜ **נשאר לעשות**: **build סופי חדש** (כולל: תיקון ה-crash + עיצוב Liquid Glass שהצטבר) → `eas submit` → לבדוק שוב ב-TestFlight שהאפליקציה לא קורסת + בודקים Sign in with Apple/Google/ATT prompt בפועל + לצלם מסכים אמיתיים מהמכשיר → תזכורת יומן לחידוש Apple client secret (מרץ 2027) → Submit ל-Review.
