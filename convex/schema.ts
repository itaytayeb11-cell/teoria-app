...
export default defineSchema({
  ...
  // ==========================================================================
  // קאש של סטטיסטיקות מאגר השאלות (גודל + פילוח לפי נושא), לפי סוג רישיון.
  // בלי זה, כל טעינה של דף הבית/רשימת נושאים הייתה סורקת את כל 1,800+
  // השאלות המלאות (טקסט+4 תשובות+הסבר) רק כדי לספור — זה מה שגרם לחריגה
  // בפועל ממכסת ה-bandwidth של Convex Free plan (793MB/חודש מ-getHome בלבד,
  // 2026-09-15). מתעדכן ב-questions.recomputeBankStats, שרץ פעם אחת אחרי
  // כל ייבוא/שינוי של מאגר השאלות (לא בכל טעינת מסך).
  // ==========================================================================
  questionBankStats: defineTable({
    licenseKey: v.string(), // "all" או סוג רישיון ("B"/"A"/"C1" וכו')
    bankSize: v.number(),
